package com.bdtask.restoraposroomdbtab.Printer.PrinterUtil;

import static com.bdtask.restoraposroomdbtab.Printer.PrinterUtil.BytesUtil.byteMerger;
import static com.bdtask.restoraposroomdbtab.Printer.PrinterUtil.BytesUtil.getBytesFromBitMap;
import static com.bdtask.restoraposroomdbtab.Printer.PrinterUtil.BytesUtil.getBytesFromDecString;
...
public class ESCUtil {
	...
	public static byte[] getPrintBarCode(String data, int symbology, int height, int width, int textposition){
		...
		try{
			...
			byte[] barcode;
			if(symbology == 10){
				barcode = getBytesFromDecString(data);
			}else{
	...
	*	需要设置1B 33 00将行间距设为0
	...
	public static byte[] selectBitmap(Bitmap bitmap, int mode){
		return byteMerger(byteMerger(new byte[]{0x1B, 0x33, 0x00}, getBytesFromBitMap(bitmap, mode)), new byte[]{0x0A, 0x1B, 0x32});
	}
