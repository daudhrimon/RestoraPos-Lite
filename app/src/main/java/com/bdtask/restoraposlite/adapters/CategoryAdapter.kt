...
class CategoryAdapter(
    ...
    inner class CategoryVH(val binding: VhFoodRecyclerBinding) :
        RecyclerView.ViewHolder(binding.root) {/**/ }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CategoryVH {
        return CategoryVH(
            VhFoodRecyclerBinding.bind(
                LayoutInflater.from(context).inflate(R.layout.vh_food_recycler, parent, false)
            )
        )
    }
    ...
    override fun onBindViewHolder(holder: CategoryVH, position: Int) {
        ...
        if (position == 0) {
            ...
            for (i in foodList.indices) {
                if (foodList[i].category == cateList[position]) {
        ...
        searchEt.doOnTextChanged { text, start, before, count ->
            ...
            if (text.toString() != "" && text.toString().isNotEmpty()) {
                ...
                for (i in tempFoList.indices) {
                    if (tempFoList[i].title.lowercase().contains(text.toString().lowercase())) {
                ...
                if (srsList.isNotEmpty()) {
                    ...
                    holder.binding.foodRecycler.adapter =
                        FoodAdepter(context, srsList, foodClickListener)
                } else {
    ...
    private fun setFoodAdapter(holder: CategoryVH, tempFoList: List<Food>) {
        if (tempFoList.isNotEmpty()) {
            ...
            holder.binding.foodRecycler.visibility = View.VISIBLE
            holder.binding.foodRecycler.adapter =
                FoodAdepter(context, tempFoList, foodClickListener)
        } else {
            holder.binding.foodRecycler.visibility = View.GONE
            holder.binding.emptyBox.visibility = View.VISIBLE
        }
